...
const initAMap = async () => {
  try {
    ...
    let mapJsKey = getRuntimeMapJsKey()
    
    // 如果 localStorage 没有缓存，从后端获取
    if (!mapJsKey) {
      try {
        const backendSettings = await getBackendRuntimeSettings()
        if (backendSettings.vite_amap_web_js_key) {
          mapJsKey = backendSettings.vite_amap_web_js_key
          const { setRuntimeMapJsKey: syncKey } = await import('@/services/api')
          syncKey(mapJsKey)
        }
      } catch (err) {
        console.warn('从后端获取高德地图配置失败:', err)
      }
    }
